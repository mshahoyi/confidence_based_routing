...
import importlib
import routellm.evals.benchmarks
importlib.reload(routellm.evals.benchmarks)
from routellm.evals.benchmarks import GSM8K, MMLU, MTBench
...
def evaluate(**args):
    ...
    args = SafeNamespace(**args)
    
    # Assign defaults
    args.output = args.output if args.output else "."
    args.overwrite_cache = args.overwrite_cache if args.overwrite_cache else []
    args.parallel = args.parallel if args.parallel else psutil.cpu_count(logical=False)
    args.strong_model = args.strong_model if args.strong_model else "gpt-4-1106-preview"
    args.weak_model = args.weak_model if args.weak_model else "mistralai/Mixtral-8x7B-Instruct-v0.1"
    args.config = args.config if args.config else None
    args.num_results = args.num_results if args.num_results else 10
    args.random_iters = args.random_iters if args.random_iters else 10
...
if __name__ == "__main__":
    ...
    evaluate(**args, config=yaml.safe_load(open(args.config, "r")) if args.config else None,)
